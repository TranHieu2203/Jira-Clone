...
public sealed class SprintDbContextFactory : IDesignTimeDbContextFactory<SprintDbContext>
{
    public SprintDbContext CreateDbContext(string[] args)
    {
        ...
        DbContextOptionsBuilder<SprintDbContext> builder = new();
        builder.UseConfiguredDatabase(cfg, migrationsAssembly: typeof(SprintDbContextFactory).Assembly.GetName().Name);

        return new SprintDbContext(builder.Options);
    }
}
