...
**Reference làm template**: [src/Modules/Sample/](src/Modules/Sample). Khi thêm module mới, copy-paste cấu trúc Sample, đổi tên.
...
```csharp
// Sample.Application/ISampleUnitOfWork.cs
public interface ISampleUnitOfWork : IUnitOfWork { }

// Sample.Infrastructure/SampleUnitOfWork.cs
...
// Sample.Api/SampleModule.cs
services.AddScoped<ISampleUnitOfWork, SampleUnitOfWork>();
```

Service inject `ISampleUnitOfWork`, **không bao giờ** inject `IUnitOfWork` raw.

Reference: [ISampleUnitOfWork.cs](src/Modules/Sample/Sample.Application/ISampleUnitOfWork.cs), [IIdentityUnitOfWork.cs](src/Modules/Identity/Identity.Application/IIdentityUnitOfWork.cs).
...
**Bug đã đạp**: `EnsureCreatedAsync()` short-circuit nếu **database** đã tồn tại — Sample tạo schema trước, Identity gọi sau thấy DB tồn tại → skip → seed crash vì `identity.roles` không có.
...
Tham chiếu: [products.page.ts `confirmDelete`](frontend/src/app/features/sample/products.page.ts).
...
- Reference: [product.service.ts](frontend/src/app/features/sample/product.service.ts).
...
curl -s http://localhost:4200/api/v1/products -H "Authorization: Bearer $TOKEN"
```
...
```bash
docker exec jira-clone-postgres psql -U jira_clone -d jira_clone -c "SELECT * FROM sample.products;"
```
...
```bash
docker exec jira-clone-postgres psql -U jira_clone -d jira_clone -c "DELETE FROM sample.products;"
```
...
1. **Tạo csproj structure**: copy `src/Modules/Sample/`, đổi tên `Sample` → `{Module}` ở: folder, csproj, RootNamespace, AssemblyName.
...
| 2 | Login API trả 200 nhưng product không persist sau Create | `IUnitOfWork` đăng ký 2 lần → DI last-wins → ProductService gọi nhầm IdentityDbContext | Typed UoW per module | [SampleUnitOfWork.cs](src/Modules/Sample/Sample.Infrastructure/SampleUnitOfWork.cs), [IdentityUnitOfWork.cs](src/Modules/Identity/Identity.Infrastructure/IdentityUnitOfWork.cs) |
...
| 6 | Confirm dialog xóa hiện text Anh "Delete X?" | Hardcode template string | `translate.get([keys], args)` rồi pass header/message/labels | [products.page.ts](frontend/src/app/features/sample/products.page.ts) `confirmDelete` |
...
# Inspect DB
docker exec jira-clone-postgres psql -U jira_clone -d jira_clone -c "\dt sample.*"
docker exec jira-clone-postgres psql -U jira_clone -d jira_clone -c "SELECT * FROM sample.products;"

# API logs
docker logs jira-clone-api --tail 50

# Reset products (giữa các test e2e)
docker exec jira-clone-postgres psql -U jira_clone -d jira_clone -c "DELETE FROM sample.products;"
...
1. Tìm pattern tương tự trong module **Sample** — nó là reference implementation.
